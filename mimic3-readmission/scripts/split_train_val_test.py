...
for id, cv in enumerate(cvs):
    ...
    with open(out_dir+str(id)+"_train_listfile801010.csv" , "w") as train_listfile:
        train_listfile.write(header)
        for line in train_lines:
            train_listfile.write(line)

    with open(out_dir+str(id)+"_val_listfile801010.csv", "w") as val_listfile:
        val_listfile.write(header)
        for line in val_lines:
            val_listfile.write(line)

    with open(out_dir+str(id)+"_test_listfile801010.csv" , "w") as test_listfile:
